...
class EpidemySuite extends FunSuite {
  ...
  test("find correct neighbours of room(0,0)") {
    val es = new EpidemySimulator
  ...
  test("find correct neighbours of room(7,7)") {
    val es = new EpidemySimulator
  ...
  test("find correct neighbours of room(7,0)") {
    val es = new EpidemySimulator
  ...
  test("find correct neighbours of room(0,7)") {
    val es = new EpidemySimulator
  ...
  test("find all neighbours of room(0,0)") {
    val es = new EpidemySimulator
    val room = es.rooms(0)(0)
    val left = room.toLeft // Room(0,7)
    val right = room.toRight // Room(0,1)
    val up = room.toUp // Room(1,0)
    val down = room.toDown // Room(7,0)
    val neighbours = room.neighbours
    assert(neighbours.size == 4);
    assert(neighbours.contains(left))
    assert(neighbours.contains(right))
    assert(neighbours.contains(up))
    assert(neighbours.contains(down))
  }
  ...
  test("room is not infected") {
    ...
    assert(!room.isInfectious, "room is not infected")
  }
  ...
  test("room is infected by a sick person") {
    ...
    person.immune = false
    person.dead = false
    assert(room.isInfectious, "room is infected by a sick person")
  }
  ...
  test("room is infected by a dead person") {
    ...
    person.immune = false
    person.dead = true
    assert(room.isInfectious, "room is infected by a dead person")
  }

  test("move a person to another room") {
    val es = new EpidemySimulator
    val person = es.persons.head
    val room = es.rooms(person.row)(person.col)
    val left = room.toLeft 
    val right = room.toRight
    val up = room.toUp
    val down = room.toDown

    // Infect all the people in three neighbour rooms, left/right/down
    for (p <- left.people) p.sick = true
    for (p <- right.people) p.sick = true
    for (p <- down.people) p.sick = true

    // The only un-infected room is up room
    person.move
    assert(person.row == up.row && person.col == up.col)
  }

  test("cannot move a person to another room") {
    val es = new EpidemySimulator
    val person = es.persons.head
    val room = es.rooms(person.row)(person.col)
    val left = room.toLeft 
    ...
    // Infect all the people in all neighbour rooms
    for (p <- left.people) p.sick = true
    for (p <- right.people) p.sick = true
    for (p <- up.people) p.sick = true
    for (p <- down.people) p.sick = true

    // A person cannot move when he is surrounded by visibly infectious people
    person.move
    assert(person.row == room.row && person.col == room.col)
  }
